...
def register_class(cls_name, cls, is_unique=True, skip_store=False):
    """
    This function registers given class into tpDcc-dccs-max module
    ...
    if REGISTER_ATTR not in tpDcc.dccs.max.__dict__:
        tpDcc.dccs.max.__dict__[REGISTER_ATTR] = list()

    if not is_unique and cls_name in tpDcc.dccs.max.__dict__:
        return

    tpDcc.dccs.max.__dict__[cls_name] = cls
    if not skip_store:
        tpDcc.dccs.max.__dict__[REGISTER_ATTR].append(cls_name)
...
def cleanup():
    ...
    for cls_name in tpDcc.dccs.max.__dict__[REGISTER_ATTR]:
        if cls_name not in tpDcc.dccs.max.__dict__:
            continue
        print('Deleting: {}'.format(cls_name))
        del tpDcc.dccs.max.__dict__[cls_name]
    del tpDcc.dccs.max.__dict__[REGISTER_ATTR]
